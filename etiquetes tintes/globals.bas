...
Public Function IsFormLoaded(FormToCheck As Form) As Integer
    ...
    IsFormLoaded = False
End Function
Public Function IsFormVisible(FormToCheck As Form) As Boolean
    Dim Y As Integer
    
    For Y = 0 To Forms.Count - 1
    If Forms(Y) Is FormToCheck Then
        If Forms(Y).visible Then IsFormVisible = True
        Exit Function
        End If
    Next
    IsFormVisible = False
