...
Begin VB.Form formordreimpresio 
   ...
   Begin VB.Timer Timer2 
      ...
      Top             =   7800
   End
   Begin VB.CheckBox CheckvalorsdeltaE 
      Caption         =   "Veure valors DeltaE"
      Height          =   210
      Left            =   330
      TabIndex        =   48
      Top             =   -15
      Width           =   2025
   End
   Begin VB.Timer timerbotocomentaris 
      Enabled         =   0   'False
      Interval        =   500
      Left            =   720
      Top             =   9270
   ...
   Begin VB.Frame framellegenda 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.Label Label15 
            ...
            Caption         =   "Subratllat falta material laminadora."
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   9.75
               Charset         =   0
   ...
   Begin VB.Timer Timer1 
      ...
      Top             =   6990
   End
   Begin VB.CommandButton bPDFmodificacions 
      BackColor       =   &H0080FF80&
      Height          =   690
      Left            =   30
   ...
   Begin VB.Menu mopcions 
      ...
      End
      Begin VB.Menu mdemanarURGENT 
         Caption         =   "Demanar bobines URGENT"
         Begin VB.Menu mdemanarcomandaurgent 
            Caption         =   "Demanar comanda URGENT"
         End
         Begin VB.Menu mllistacomandesurgents 
            Caption         =   "Llista de comandes URGENTS"
         End
      End
   End
   ...
   Begin VB.Menu mllegenda 
      Caption         =   "Llegenda"
   End
   Begin VB.Menu mlectura 
      Caption         =   "Lectura distancia"
      Visible         =   0   'False
      Begin VB.Menu mnumport 
         Caption         =   "Numero de Port "
      End
      Begin VB.Menu mresetdistancia 
         Caption         =   "Reset distancia tubbase"
      End
      Begin VB.Menu mlecturadiambobina 
         Caption         =   "Lectura diametre bobina"
      End
   End
End
...
Private Sub bbobinesamaquina_Click()
  ...
  ' MsgBox atrim(vnumc)
   If notepackinglist(vnumc) Then
       ...
       reixa.col = 0
       If cadbl(reixa.TextMatrix(reixa.row, reixa.col)) = vnumc Then
           If reixa.CellBackColor = &H80FF80 Then
                reixa.CellBackColor = QBColor(15)
               Else: reixa.CellBackColor = &H80FF80 'si hi ha totes les bobines A MAQUINA  poso la comanda amb verd txillon
           End If
       End If
...
Private Sub bcanvimaquina_Click()
  ...
  Dim vnommaq As String
   Timer1.Enabled = False: timerbotocomentaris.Enabled = False
  Unload formseleccio
  Load formseleccio
...
surtir:
  ...
  End If
  Unload formseleccio
   Timer1.Enabled = True: timerbotocomentaris.Enabled = True
End Sub
...
Private Sub CheckvalorsdeltaE_Click()
   If CheckvalorsdeltaE.Value = 0 Then Unload formcanvisanilox
End Sub
...
Function esvalidaaquestacomanda(comanda As Double) As Boolean
    ...
    If Not form1.comandavalida(cadbl(comanda), msg, True) Then
          ...
          If InStr(1, UCase(msg), "STANDBY") > 0 Then MsgBox msg, vbCritical, "Atenció": comanda = "0": esvalidaaquestacomanda = False: GoTo fi
...
Function faltamaterialdelcomplexa(vnumc As Double) As Boolean
  ...
  If Not rst.EOF Then
      vlinkcomanda1 = cadbl(rst!linkcomanda1)
      vlinkcomanda2 = cadbl(rst!linkcomanda2)
      If vlinkcomanda1 <> 0 Then
          Set rst = dbtmp.OpenRecordset("select producte,comanda from comandes where comanda=" + atrim(vlinkcomanda1))
          If Not rst.EOF Then
               If rst!producte <> "PC" Then GoTo fi
                Set rst = dbstocks.OpenRecordset("SELECT Parcials.comanda, Palets.Dataactivacio FROM Palets INNER JOIN Parcials ON Palets.Idpalet = Parcials.idpalet where parcials.comanda='" + atrim(rst!comanda) + "'")
                If Not rst.EOF Then
                    If atrim(rst!dataactivacio) = "" Then faltamaterialdelcomplexa = True
                      Else: faltamaterialdelcomplexa = True
                End If
          End If
      End If
      If vlinkcomanda2 <> 0 Then
          Set rst = dbtmp.OpenRecordset("select producte,comanda from comandes where comanda=" + atrim(vlinkcomanda2))
          If Not rst.EOF Then
               If rst!producte <> "PC" Then GoTo fi
                Set rst = dbstocks.OpenRecordset("SELECT Parcials.comanda, Palets.Dataactivacio FROM Palets INNER JOIN Parcials ON Palets.Idpalet = Parcials.idpalet where parcials.comanda='" + atrim(rst!comanda) + "'")
                If Not rst.EOF Then
...
Sub carregar_reixaordre(Optional vnumc As Double)
  ...
  While Not rst.EOF
    ...
    If UCase(arguments(1)) = "DESBOBINADORS" Then GoTo colorbobinesCOMANDES
...
colorbobinesCOMANDES:
    ...
    If Not esmuntadora Then
      ...
      If vtoteslesbobines = 4 Then
           reixa.CellBackColor = &HF3B378 ' blau si no hi ha prou material
      End If
      ...
      If reixa.col = 0 Then If faltamaterialdelcomplexa(rst!comanda) Then reixa.CellFontUnderline = True
      If UCase(arguments(1)) = "DESBOBINADORS" Then GoTo proxima
     ' If rst!imprespackinglist Then reixa.col = 0: reixa.CellBackColor = &H5C31DD 'vermell xulu si ja han impres packinglist
      ...
      End If
        Else
           If clixesnomarcats(rst!comanda) Then reixa.col = 2: reixa.CellBackColor = &H5C31DD: reixa.col = 0
    End If
proxima:
    vrow = vrow + 1
    rst.MoveNext
...
Unload veurereport
If Not esmuntadora Then Unload formcanvisanilox
End Sub
...
Private Sub mdemanarcomandaurgent_Click()
  Dim vcomanda As Double
  Dim rst As Recordset
  vcomanda = cadbl(reixa.TextMatrix(reixa.row, 0))
  vcomanda = cadbl(InputBox("Vols demanar URGENT AQUESTA COMANDA?", "ATENCIO", atrim(vcomanda)))
  If vcomanda > 0 Then
      Set rst = dbtmp.OpenRecordset("select * from impresores_ordreimpresio where comanda=" + atrim(vcomanda))
      If rst.EOF Then MsgBox "LA COMANDA " + atrim(vcomanda) + " NO ESTÀ DINS LA LLISTA DE ORDRE IMPRESIÓ", vbCritical, "ATENCIÓ": GoTo fi
      rst.Edit: rst!urgent = True: rst.Update
      MsgBox "COMANDA MARCADA COM URGENT" + vbNewLine + vbNewLine + "AVISA ALS TORERUS.", vbExclamation, "URGENT"
  End If
fi:
  Set rst = Nothing
End Sub
...
Private Sub mlecturadiambobina_Click()
     Dim vDistanciaTubBase As Double
     Dim vdistancia As String
     vDistanciaTubBase = cadbl(llegir_ini("SensorDistancia", "DistanciaTubBase", "comandes.ini"))
     vdistancia = llegir_sensordistancia
     MsgBox atrim(vdistancia - vDistanciaTubBase) + " Cm"
End Sub
...
Private Sub mllistacomandesurgents_Click()
  Unload formseleccio
   Load formseleccio
  formseleccio.Data1.DatabaseName = rutadelfitxer(cami) + "BAIXES.mdb"
  formseleccio.Data1.RecordSource = "select comanda from impresores_ordreimpresio where urgent=true order by comanda"
  formseleccio.refrescar
  formseleccio.caption = "COMANDES URGENTS"
  formseleccio.DBGrid2.Columns(0).width = 2500
  formseleccio.Show 1
  If seleccioret = 1 And formseleccio.DBGrid2.Columns(0) <> "" Then
     If MsgBox("Vols treure la comanda " + atrim(formseleccio.DBGrid2.Columns(0)) + " de la llista d'URGENTS?", vbCritical + vbDefaultButton2 + vbYesNo, "ATENCIÓ") = vbYes Then
         dbtmp.Execute "update impresores_ordreimpresio set urgent=false where comanda=" + atrim(formseleccio.DBGrid2.Columns(0))
     End If
  End If
  Unload formseleccio
End Sub
...
Private Sub mnumport_Click()
   Dim vport As String
   vport = llegir_ini("SensorDistancia", "Port", "comandes.ini")
   If vport = "{[}]" Then vport = "0"
   vport = cadbl(InputBox("Entra el numero de port que vols utilitzar per llegir distancia:", "Port Serie"))
   If vport > 0 Then escriure_ini "SensorDistancia", "Port", vport, "comandes.ini"
End Sub
...
Private Sub mpossarcomentari_Click()
  ...
  End If
  If bcomentariscomanda.tag <> "" Then obsidtreball.obsid = bcomentariscomanda.tag + vbNewLine + obsidtreball.obsid
  ...
  If arguments(1) = "" Then
        obsidtreball.Left = 100
        obsidtreball.Top = formordreimpresio.Top + (formordreimpresio.Height / 2)
         Else: obsidtreball.Left = 0 ' si es 0 desde el formulari es centrarà a pantalla.
  End If
  obsidtreball.Show 1
...
End Sub
Sub botoPDFmodificacions(vActivar As Boolean)
...
Private Sub mresetdistancia_Click()
   Dim vport As String
   Dim vdistancia As Double
   vport = llegir_ini("SensorDistancia", "Port", "comandes.ini")
   If vport = "{[}]" Or cadbl(vport) = 0 Then MsgBox "No hi ha port assignat": Exit Sub
   vdistancia = llegir_sensordistancia
   If vdistancia > 0 Then escriure_ini "SensorDistancia", "DistanciaTubBase", atrim(vdistancia), "comandes.ini": MsgBox "Distancia Tub Base=" + atrim(vdistancia) + " Cm"
End Sub
Function llegir_sensordistancia() As Double
  Dim vport As String
  Dim vdistancia As Double
  Dim sData As String
  Dim vdata As Date
  ...
  If MSComm1.PortOpen = False Then
        vport = llegir_ini("SensorDistancia", "Port", "comandes.ini")
        If cadbl(vport) = 0 Then Exit Function
        MSComm1.CommPort = vport
  ...
  End If
  vdata = Now
  While MSComm1.InBufferCount = 0 And DateDiff("s", vdata, Now) < 5
          DoEvents
  Wend
  sData = MSComm1.Input ' Llegir les dades del buffer d'entrada
  llegir_sensordistancia = cadbl(Mid(sData, 1, InStr(1, sData, vbNewLine)))
  
End Function
Private Sub reixa_Click()
  If CheckvalorsdeltaE.Value = 1 Then
        ...
        formcanvisanilox.tag = "nomesdelta " + atrim(formcanvisanilox.ultimacomandaimpresa(CByte(etmaquina.tag)))
        formcanvisanilox.tag = "nomesdelta " + atrim(IIf(cadbl(reixa.TextMatrix(reixa.row - 1, 0)), cadbl(reixa.TextMatrix(reixa.row - 1, 0)), 0))
        formcanvisanilox.tag = formcanvisanilox.tag + reixacomandesseguents
        formcanvisanilox.Show
        If Screen.Height < (formordreimpresio.Height + formcanvisanilox.Height) Then
            formcanvisanilox.Left = formordreimpresio.Left + formordreimpresio.width + 100
            formcanvisanilox.Top = formordreimpresio.Top + formordreimpresio.Height - formcanvisanilox.Height
              Else
                formcanvisanilox.Left = formordreimpresio.Left
                formcanvisanilox.Top = formordreimpresio.Top + formordreimpresio.Height
        End If
  End If
End Sub
Function reixacomandesseguents() As String
  For i = reixa.row To reixa.row + 2
     If reixa.Rows > i Then reixacomandesseguents = reixacomandesseguents + "," + reixa.TextMatrix(i, 0)
  Next i
End Function
...
Sub reixa_refrescarfila()
   ...
   If Not rst.EOF Then
    ...
    End If
    possar_comentaricomandasicorrespon cadbl(numerodecomandaseleccionada)
...
Private Sub reixa_RowColChange()
   reixa_refrescarfila
   Unload veurereport
      timerbotocomentaris.Enabled = True
End Sub
...
Private Sub Timer2_Timer()
  Me.caption = atrim(llegir_sensordistancia) + " Cm"
End Sub

Private Sub timerbotocomentaris_Timer()
   If bcomentariscomanda.visible And nummaq > 0 And arguments(1) = "" Then mpossarcomentari_Click
   timerbotocomentaris.Enabled = False
