...
Global estemfentreprint As Boolean
Global nomordinadorcomexi As String
Global id_treball As Double
Global ordremodificacio As Double
Global dbclixes As Database
Global ruta_documentacio_clixes As String
Global noespota0 As Boolean
Global dbpantones As Database
  Global rstpantones As Recordset
...
Global dbmissatges As Database
...
Global metrescomanda As Double
...
Global nommaq As String
Global numop As Byte
Global numop2 As Byte
...
Function cvdecimal(v As String) As String
   ...
   cvdecimal = v
End Function
Function treurecaracter(v As String, c As String) As String
    Dim pos As Byte
   pos = InStr(1, v, c)
   If pos > 0 Then
      treurecaracter = Mid(v, 1, pos - 1) + Mid(v, pos + 1)
        Else
          treurecaracter = v
   End If
End Function
...
Sub wait(segonsespera As Byte)
  ...
  Wend
End Sub
Sub esperar(vmilisegons As Double)
    IniTime = GetTickCount()
    While GetTickCount() < (IniTime + vmilisegons)
        DoEvents
    Wend
