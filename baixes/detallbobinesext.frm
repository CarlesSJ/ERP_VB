...
Begin VB.Form detallbobext 
   Caption         =   "Detall de Bobines Extrussores"
   ClientHeight    =   5580
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5685
   Icon            =   "detallbobinesext.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton sortir 
      Height          =   450
      Left            =   5145
      Picture         =   "detallbobinesext.frx":0442
      Style           =   1  'Graphical
      TabIndex        =   4
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton alta 
      Height          =   450
      Left            =   255
      Picture         =   "detallbobinesext.frx":0944
      Style           =   1  'Graphical
      TabIndex        =   3
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton eliminar 
      Height          =   450
      Left            =   840
      Picture         =   "detallbobinesext.frx":0D76
      Style           =   1  'Graphical
      TabIndex        =   2
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton gravar 
      Height          =   450
      Left            =   4635
      Picture         =   "detallbobinesext.frx":1088
      Style           =   1  'Graphical
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin VB.Data Data1 
      ...
      RecordSource    =   "bobinesext"
      Top             =   75
      Visible         =   0   'False
      Width           =   1245
   End
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "detallbobinesext.frx":13CA
      Height          =   4860
      Left            =   210
      OleObjectBlob   =   "detallbobinesext.frx":13DA
      TabIndex        =   0
      Top             =   510
      Width           =   5445
   End
   ...
   End
End
Attribute VB_Name = "detallbobext"
...
Sub nova_bobina()
  ...
  End If
  Set rsttmp2 = dbtmpb.OpenRecordset("select id  from extrussores where comanda=" + atrim(Extrussores.bobines.Recordset!comanda))
  elgran = 0
  While Not rsttmp2.EOF
   Set rstmp = dbtmpb.OpenRecordset("select max(numerodebobina) as elgran from bobinesext where controlid=" + atrim(rsttmp2!id))
   If Not rstmp.EOF Then
      If cadbl(rstmp!elgran) > elgran Then elgran = cadbl(rstmp!elgran)
   End If
   rsttmp2.MoveNext
  Wend
  Set rstmp = dbtmpb.OpenRecordset("select * from bobinesext where controlid=" + atrim(Extrussores.bobines.Recordset!id) + " and numerodebobina=" + atrim(elgran))
  data1.Recordset.AddNew
  data1.Recordset!numerodebobina = elgran + 1
  data1.Recordset!controlid = atrim(Extrussores.bobines.Recordset!id)
  data1.Recordset!numempalmes = 0
  col = 0
  If Not rstmp.EOF Then
     data1.Recordset!operari1 = rstmp!operari1
     data1.Recordset!operari2 = rstmp!operari2
     data1.Recordset!metres = rstmp!metres
     data1.Recordset!kilos = rstmp!kilos
     
     If rstmp!operari1 > 0 Then col = IIf(Check1.Value = 1, 3, 4)
   Else: col = 0
  End If
  data1.Recordset.Update
  'Data1.Refresh
...
Private Sub DBGrid1_KeyDown(KeyCode As Integer, Shift As Integer)
  ...
  saltacol = 4
  If Check1.Value = 1 Then saltacol = 3
  If KeyCode = 13 Then
    KeyCode = 0
    ...
    End If
  End If
  If KeyCode = 113 And (DBGrid1.col = 0 Or DBGrid1.col = 1) Then
    triaroperaris
  End If
End Sub

Private Sub DBGrid1_LostFocus()
'If cadbl(DBGrid1.Columns(3).Text) = 0 And cadbl(DBGrid1.Columns(4).Text) = 0 Then
...
'comprova si hem escrit el numero amb separat per .
  ...
  End If
    ...
    If DBGrid1.Text = "0" Then DBGrid1.SelLength = Len(DBGrid1.Text)
    If (LastCol = 1 Or LastCol = 0) Then
      If cadbl(DBGrid1.Columns(LastCol)) <> 0 Then
       Set rsttmp = dbtmp.OpenRecordset("select codi from operaris where maquina='E' and codi=" + atrim(cadbl(DBGrid1.Columns(LastCol))))
...
Sub triaroperaris()
  ...
  formseleccio.data1.RecordSource = "select * from operaris where maquina='E'"
...
data1.RecordSource = "select * from bobinesext where controlid=" + atrim(cadbl(Extrussores.bobines.Recordset!id)) + " order by numerodebobina"
