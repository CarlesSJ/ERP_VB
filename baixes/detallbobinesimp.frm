...
Begin VB.Form detallbobimp 
   Caption         =   "Detall de Bobines Impressores"
   ClientHeight    =   5580
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6900
   Icon            =   "detallbobinesimp.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame bobent 
      ...
      Begin MSDBGrid.DBGrid DBGrid2 
         Bindings        =   "detallbobinesimp.frx":0442
         Height          =   2775
         Left            =   15
         OleObjectBlob   =   "detallbobinesimp.frx":0457
         TabIndex        =   8
         Top             =   225
         Width           =   2205
      End
   ...
   Begin VB.CommandButton sortir 
      Height          =   450
      Left            =   5145
      Picture         =   "detallbobinesimp.frx":0E3A
      Style           =   1  'Graphical
      TabIndex        =   4
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton alta 
      Height          =   450
      Left            =   255
      Picture         =   "detallbobinesimp.frx":133C
      Style           =   1  'Graphical
      TabIndex        =   3
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton eliminar 
      Height          =   450
      Left            =   840
      Picture         =   "detallbobinesimp.frx":176E
      Style           =   1  'Graphical
      TabIndex        =   2
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton gravar 
      Height          =   450
      Left            =   4665
      Picture         =   "detallbobinesimp.frx":1A80
      Style           =   1  'Graphical
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "detallbobinesimp.frx":1DC2
      Height          =   4860
      Left            =   210
      OleObjectBlob   =   "detallbobinesimp.frx":1DD2
      TabIndex        =   0
      Top             =   510
      Width           =   6615
   End
   ...
   End
End
Attribute VB_Name = "detallbobimp"
...
Sub nova_bobina()
  ...
  End If
  Set rsttmp2 = dbtmpb.OpenRecordset("select id  from impressores where comanda=" + atrim(Impressores.bobines.Recordset!comanda))
  elgran = 0
  While Not rsttmp2.EOF
   ...
   rsttmp2.MoveNext
  Wend
  Set rstmp = dbtmpb.OpenRecordset("select * from bobinesimp where controlid=" + atrim(Impressores.bobines.Recordset!ID) + " and numerodebobina=" + atrim(elgran))
  data1.Recordset.AddNew
  data1.Recordset!numerodebobina = elgran + 1
  data1.Recordset!controlid = atrim(Impressores.bobines.Recordset!ID)
...
databobent.RecordSource = "select * from bobinesentimp where id=" + atrim(cadbl(data1.Recordset!ID))
databobent.Refresh
End Sub
...
'If cadbl(DBGrid1.Columns(3).Text) = 0 Then
  ...
  End If
    ...
    If LastCol = 0 Then
      If cadbl(DBGrid1.Columns(LastCol)) <> 0 Then
       Set rsttmp = dbtmp.OpenRecordset("select codi from operaris where maquina='I' and codi=" + atrim(cadbl(DBGrid1.Columns(LastCol))))
...
Private Sub eliminar_Click()
  If MsgBox("Segur que vols borrar aquesta bobina?", vbCritical + 4, "Atenció") = vbYes Then
     If Not data1.Recordset.EOF Then
       dbtmpb.Execute "delete * from impempalmes where id=" + atrim(cadbl(data1.Recordset!ID))
       dbtmpb.Execute "delete * from bobentradaimp where idbobimp=" + atrim(cadbl(data1.Recordset!ID))
...
data1.RecordSource = "select * from bobinesimp where controlid=" + atrim(Impressores.bobines.Recordset!ID) + " order by numerodebobina"
databobent.DatabaseName = cami
databobent.RecordSource = "bobentradaimp"
data1.Refresh
End Sub
