...
Begin VB.Form detallbobsol 
   Caption         =   "Detall de Bobines Soldadores"
   ClientHeight    =   5580
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5685
   ControlBox      =   0   'False
   Icon            =   "detallbobinessol.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton sortir 
      Height          =   450
      Left            =   5145
      Picture         =   "detallbobinessol.frx":0442
      Style           =   1  'Graphical
      TabIndex        =   4
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton alta 
      Height          =   450
      Left            =   255
      Picture         =   "detallbobinessol.frx":0944
      Style           =   1  'Graphical
      TabIndex        =   3
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton eliminar 
      Height          =   450
      Left            =   840
      Picture         =   "detallbobinessol.frx":0D76
      Style           =   1  'Graphical
      TabIndex        =   2
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton gravar 
      Height          =   450
      Left            =   4635
      Picture         =   "detallbobinessol.frx":1088
      Style           =   1  'Graphical
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "detallbobinessol.frx":13CA
      Height          =   4860
      Left            =   210
      OleObjectBlob   =   "detallbobinessol.frx":13DA
      TabIndex        =   0
      Top             =   510
      Width           =   5445
   End
   ...
   End
End
Attribute VB_Name = "detallbobsol"
...
Sub nova_bobina()
  ...
  End If
  Set rsttmp2 = dbtmpb.OpenRecordset("select id  from Soldadores where comanda=" + atrim(Soldadores.bobines.Recordset!comanda))
  elgran = 0
  While Not rsttmp2.EOF
   Set rstmp = dbtmpb.OpenRecordset("select max(numerodesac) as elgran from bobinessol where controlid=" + atrim(rsttmp2!ID))
   If Not rstmp.EOF Then
      If cadbl(rstmp!elgran) > elgran Then elgran = cadbl(rstmp!elgran)
   End If
   rsttmp2.MoveNext
  Wend
  Set rstmp = dbtmpb.OpenRecordset("select * from bobinessol where controlid=" + atrim(Soldadores.bobines.Recordset!ID) + " and numerodesac=" + atrim(elgran))
  data1.Recordset.AddNew
  data1.Recordset!numerodesac = elgran + 1
  data1.Recordset!controlid = atrim(Soldadores.bobines.Recordset!ID)
  'Data1.Recordset!numempalmes = 0
  col = 0
  If Not rstmp.EOF Then
     data1.Recordset!operari1 = rstmp!operari1
'     Data1.Recordset!metres = rstmp!metres
     data1.Recordset!unitatsxsac = rstmp!unitatsxsac
     data1.Recordset!palet = rstmp!palet
     'If rstmp!operari1 > 0 Then col = IIf(Check1.Value = 1, 2, 3)
     col = 2
   Else: col = 0: data1.Recordset!palet = 1
  End If
  If IsNull(data1.Recordset!palet) Then data1.Recordset!palet = 0
  data1.Recordset.Update
  'Data1.Refresh
  DBGrid1.Refresh
...
Sub comprovarsijahihaalgualbaranat()
   Dim rst As Recordset
   Set rst = dbtmp.OpenRecordset("Select * from bobinesent where NUMalbara<>0 AND NUMALBARA<>NULL and comanda=" + atrim(Soldadores.bobines.Recordset!comanda))
   If Not rst.EOF Then
       MsgBox "La comanda " + atrim(Soldadores.bobines.Recordset!comanda) + " ja té entregues fetes o albaranades assegureu-vos que les bosses que entreu s'afegeixin a l'albarà correctament a Expedicions.", vbCritical, "A T E N C I Ó"
   End If
End Sub
Private Sub alta_Click()
 Static unsolcop As Boolean
 If Not unsolcop Then comprovarsijahihaalgualbaranat: unsolcop = True
 nova_bobina
End Sub
...
Private Sub DBGrid1_BeforeUpdate(Cancel As Integer)
  'If cadbl(DBGrid1.Columns("palet")) = 0 Then MsgBox "Falta el numero de palet", vbCritical, "Error": Cancel = 1
End Sub
...
' If cadbl(DBGrid1.Columns(2).Text) = 0 Then
  ...
  End If
    ...
    If LastCol = 0 Then
      If cadbl(DBGrid1.Columns(LastCol)) <> 0 Then
       Set rsttmp = dbtmp.OpenRecordset("select codi from operaris where maquina='S' and codi=" + atrim(cadbl(DBGrid1.Columns(LastCol))))
...
Private Sub Form_Load()
    ...
    data1.RecordSource = "select * from bobinessol where controlid=" + atrim(cadbl(Soldadores.bobines.Recordset!ID)) + " order by numerodesac"
    data1.Refresh
End Sub
...
Private Sub gravar_Click()
    ...
    If data1.Recordset.EditMode > 0 Then data1.Recordset.Update
        If hihaalgunpaletaZero Then MsgBox "Hi ha alguna bobina sense numero de palet.", vbCritical, "Error": Exit Sub
End Sub
Function hihaalgunpaletaZero() As Boolean
   data1.Refresh
   While Not data1.Recordset.EOF
      If data1.Recordset!palet = 0 Then hihaalgunpaletaZero = True: GoTo fi
