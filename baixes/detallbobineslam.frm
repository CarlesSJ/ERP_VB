...
Begin VB.Form detallboblam 
   Caption         =   "Detall de Bobines Laminadores"
   ClientHeight    =   5580
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5685
   Icon            =   "detallbobineslam.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton sortir 
      Height          =   450
      Left            =   5145
      Picture         =   "detallbobineslam.frx":0442
      Style           =   1  'Graphical
      TabIndex        =   4
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton alta 
      Height          =   450
      Left            =   255
      Picture         =   "detallbobineslam.frx":0944
      Style           =   1  'Graphical
      TabIndex        =   3
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton eliminar 
      Height          =   450
      Left            =   840
      Picture         =   "detallbobineslam.frx":0D76
      Style           =   1  'Graphical
      TabIndex        =   2
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton gravar 
      Height          =   450
      Left            =   4635
      Picture         =   "detallbobineslam.frx":1088
      Style           =   1  'Graphical
      TabIndex        =   1
      TabStop         =   0   'False
   ...
   Begin MSDBGrid.DBGrid DBGrid1 
      Bindings        =   "detallbobineslam.frx":13CA
      Height          =   4860
      Left            =   210
      OleObjectBlob   =   "detallbobineslam.frx":13DA
      TabIndex        =   0
      Top             =   510
      Width           =   5445
   End
   ...
   End
End
Attribute VB_Name = "detallboblam"
...
Sub nova_bobina()
  ...
  End If
  Set rsttmp2 = dbtmpb.OpenRecordset("select id  from laminadores where comanda=" + atrim(Laminadores.bobines.Recordset!comanda))
  elgran = 0
  While Not rsttmp2.EOF
   ...
   rsttmp2.MoveNext
  Wend
  Set rstmp = dbtmpb.OpenRecordset("select * from bobineslam where controlid=" + atrim(Laminadores.bobines.Recordset!id) + " and numerodebobina=" + atrim(elgran))
  data1.Recordset.AddNew
  data1.Recordset!numerodebobina = elgran + 1
  data1.Recordset!controlid = atrim(Laminadores.bobines.Recordset!id)
   ...
   'On Error Resume Next
    ...
    If LastCol = 0 Then
      If cadbl(DBGrid1.Columns(LastCol)) <> 0 Then
       Set rsttmp = dbtmp.OpenRecordset("select codi from operaris where maquina='L' and codi=" + atrim(cadbl(DBGrid1.Columns(LastCol))))
...
data1.RecordSource = "select * from bobineslam where controlid=" + atrim(Laminadores.bobines.Recordset!id) + " order by numerodebobina"
