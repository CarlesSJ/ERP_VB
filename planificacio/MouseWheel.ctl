...
Attribute VB_Exposed = False
        ...
        '********************************'
        '* MOUSE WHEEL EVENT CONTROL    *'
        '*                              *'
        '* Give your applications the   *'
        '* feature of trapping the      *'
        '* mousewheel event.            *'
        '*                              *'
        '* © by Flavio González Vázquez *'
        '* http://www.flavionet.com     *'
        '* flavio@ya.com                *'
        '* This control is FREE         *'
        '*                              *'
        '* Based in Self-Subclassed     *'
        '* controls by Paul Caton       *'
        '*                              *'
        '********************************'
...
Public Event WheelMove(bDown As Boolean)

Private Sub UserControl_ReadProperties(PropBag As PropertyBag)

  If Ambient.UserMode Then                                                              'If we're not in design mode
    Call Subclass_Start(UserControl.Parent.hWnd)                                               'Start subclassing
    Call Subclass_AddMsg(UserControl.Parent.hWnd, WM_MOUSEWHEEL)
  End If
End Sub
...
Private Sub UserControl_Terminate()
  ...
  If Ambient.UserMode Then
    Call Subclass_StopAll                                                               'Stop subclassing
  End If
Catch:
End Sub
Public Sub zSubclass_Proc(ByVal bBefore As Boolean, ByRef bHandled As Boolean, ByRef lReturn As Long, ByRef lng_hWnd As Long, ByRef uMsg As Long, ByRef wParam As Long, ByRef lParam As Long)
  ...
  If uMsg = WM_MOUSEWHEEL Then
    If wParam = &HFF880000 Then
        'Label1.Caption = "v"
        RaiseEvent WheelMove(True)
    End If
    If wParam = &H780000 Then
        'Label1.Caption = "^"
        RaiseEvent WheelMove(False)
    End If
  End If
