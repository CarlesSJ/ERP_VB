...
Global programaoperaris As Boolean
Global programanomeslectura As Boolean
Global primerapestanya As String
Global poteditar As Boolean
Global ordrereixa As String
Global ultimfiltre As Integer
Global nummaquina As Byte
Global iniconfigreixa As String
Global camps(100, 4) As String
...
Global dbplanificacioalicia As Database
Global dbplanificaciooperaris As Database
Global codiKeyCode As Integer
Global kgcompralinia As Double
Global dataentrega As Date
Global preulinia As Double
Global amplelinia As Double
Global diamextlinia As Double
Global mandrillinia As Double
Global numcomanda As String
Global fitxertemp As String
Global campscontrolalicia(20, 2) As String
Global idiomaclient As String
Global taula_tmp As String
Global taulaplanificacio As String
Global fitxerini As String
Global llistadecampsvalids As String
Global campcontrol As Control
Global ultimaactualitzacio As Date
...
On Error Resume Next
  ...
  atrim = Trim(valo)
End Function
Function cadbl(ByVal valo As Variant, Optional decimals As Byte = 255) As Double
  
  valo = cvdecimal(atrim(valo))
  valo = treurecaracter(valo, ".", ",")
  If Not IsNumeric(valo) Then valo = 0
  
  If decimals < 10 Then
     valo = Redondejar(CDbl(valo), CInt(decimals))
  End If
...
End Function
Function cvdecimal(v As String) As String
   If InStr(1, v, ".") > InStr(1, v, ",") And InStr(1, v, ",") > 0 Then v = treurecaracter(v, ",")
   If InStr(1, v, ",") > InStr(1, v, ".") And InStr(1, v, ".") > 0 Then v = treurecaracter(v, ".")
   
   If InStr(1, v, ".") > 0 Then Mid(v, InStr(1, v, "."), 1) = simboldecimal
   If InStr(1, v, ",") > 0 Then Mid(v, InStr(1, v, ","), 1) = simboldecimal
           
   cvdecimal = v
End Function
Function treurecaracter(ByVal v As String, C As String, Optional canviarper As String) As String
    Dim pos As Byte
   pos = InStr(1, v, C)
   If pos > 0 Then
      treurecaracter = Mid(v, 1, pos - 1) + atrim(canviarper) + Mid(v, pos + 1)
        Else
          treurecaracter = v
   End If
End Function
Function simboldecimal() As String
   Dim vd As Double
   vd = 100 / 3
   If InStr(1, Str(vd), ".") Then
       simboldecimal = "."
      Else: simboldecimal = ","
   End If
End Function
Function simbolmiler() As String
   Dim vd As Double
   vd = 100 / 3
   If InStr(1, Str(vd), ".") Then
       simbolmiler = ","
...
Function treure_apostruf(ByVal n As String) As String
   ...
   Wend
   While InStr(n, Chr(34))
     n = Mid(n, 1, InStr(1, n, Chr(34)) - 1) + "´" + Mid(n, InStr(1, n, Chr(34)) + 1)
   Wend
   If n = "{[}]" Then n = ""
   treure_apostruf = n
End Function
...
Function micresmaterial(codimesuralineal As Byte, espesor As Double, tubolam As String) As String
 ...
 ' Dim r As String
  ...
  If descripcio = "GALGUES" Then
            If tubolam = "T" Then
                 r = Redondejar(espesor / 4, 1)
                  Else: r = Redondejar(espesor / 2, 1)
            End If
  End If
...
Function substituir(cadena As String, buscar As String, canviar As String) As String
   ...
   If InStr(1, cadena, buscar) = 0 Then substituir = cadena: Exit Function
   acaba = comença + Len(buscar) + 1
   cadena = Mid(cadena, 1, comença) + canviar + Mid(cadena, acaba)
   substituir = cadena
   'MsgBox linia
End Function
Function enviaremailgenericambadjunt(sSendTo As String, sSubject As String, sText As String, vadjunt As String) As Boolean
   Dim usuarim As String
   Dim contrasenyam As String
   Dim destinatari As String
   Dim vnomcarpeta As String
   Dim vadjuntoriginal As String
   vadjuntoriginal = vadjunt
   ...
   escriure_ini "Capcalera", "adjunt", vnomcarpeta + "\" + substituir(vadjunt, rutadelfitxer(vadjunt), ""), vnomcarpeta + "\dadesmail.txt"
   Copiar_Fitxer vadjuntoriginal, vnomcarpeta
...
Sub msgboxEx(vMissatge As String, vIcones As Double, vTitol As String, vTempsEspera As Double)
   CreateObject("WScript.Shell").Run "mshta.exe vbscript:close(CreateObject(""WScript.Shell"").Popup(""" + vMissatge + """,""" + atrim(vTempsEspera) + """,""" + vTitol + """))"
End Sub
