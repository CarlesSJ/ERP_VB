...
Dim valorsreixa(0 To 100) As camp
Sub refrescar()
...
errorr:
  ...
  If atrim(Data1.RecordSource) = "" Then Exit Sub
...
Sub guardarvalorsreixa()
  ...
  For i = 0 To DBGrid2.Columns.Count - 1
    ...
    'escriure_ini "reixatintesclixesnous", "ample" + atrim(i), atrim(valorsreixa(i).width), "amplesreixes.ini"
...
Sub carregarvalorsreixa()
  ...
  For i = 0 To DBGrid2.Columns.Count - 1
    ...
    'llegir_ini "reixatintesclixesnous", "ample" + atrim(i), "amplesreixes.ini"
  Next i
End Sub
...
Function possarfiltre()
   ...
   res = va
   If InStr(1, LCase(va), "order") Then res = Mid(va, 1, InStr(1, LCase(va), "order") - 1)
...
Private Sub DBGrid2_HeadClick(ByVal ColIndex As Integer)
  ...
  instsql = DBGrid2.tag
  If InStr(1, LCase(instsql), "order by") > 0 Then instsql = Mid(instsql, 1, InStr(1, LCase(instsql), "order by") - 1)
...
If InStr(1, Data1.RecordSource, " from ") <> 0 Then
 ...
 Else: If Trim(Data1.RecordSource) <> "" Then formseleccio.caption = "Busqueda de " + UCase(Data1.RecordSource)
End If
