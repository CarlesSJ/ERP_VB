...
Global whereultimfiltre As String
Global modificartintes As Boolean
...
Global campsestat(30, 4) As String
...
Global comandabuscada As Double
...
Dim t_Op As SHFILEOPSTRUCT
  If IsNull(opcions) Then opcions = FOF_ALLOWUNDO
    ...
    retorn = SHFileOperation(t_Op)
    If retorn <> 0 Then MsgBox "Error al copiar." + Chr(10) + "Numero d'error: " + atrim(retorn) + Chr(10) + "Possiblement el nom del fitxer d'origen o destí te algun simbol estrany."
...
Sub obrir_document(nomfitxer As String)
 ...
 If UCase(nomordinador) = "ORD-ENCARREGATI" Then
     Shell "explorer.exe " + rutadelfitxer(nomfitxer), vbNormalFocus: Exit Sub
 End If
 If InStr(1, nomfitxer, ".doc") > 0 And Not existeix(nomfitxer) Then nomfitxer = nomfitxer + "x"
...
'canvia el color dels controls al entrar a dins
 ...
 If controlcanviat.hWnd <> Screen.ActiveControl.hWnd Then
